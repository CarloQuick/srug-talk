...
System Call (syscall)
| syscall  | Page | Description | 
|:---|:---|:---|
| unshare() | unshare(2) | Allows a process to control its shared execution context without creating a new process |
| mount/umount() | mount(2)/umount(2)| Attaches or detaches a filesystem |
| execvp() | exec(3) | Replaces current process image with another (not a new process) and replicate shell PATH searching behavior |
| fork() | fork(2) | Creates a new process by duplicating the calling process. The new process is referred to as the child process. The calling process is referred to as the parent process |
| chroot() | chroot(2) | Changes the processes root directory of the process |
